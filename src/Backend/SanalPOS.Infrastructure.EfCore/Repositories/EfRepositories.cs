...
public class EfRefundTransactionRepository : EfRepository<RefundTransaction>, IRefundTransactionRepository
{
    ...
    {
        var rows = await Set.AsNoTracking()
            .Where(r => r.Status == RefundStatus.Completed && r.CreatedAt >= fromUtc && r.CreatedAt < toUtc)
            .Join(Context.Set<PaymentTransaction>(),
                r => r.OriginalTransactionId,
                t => t.Id,
                (r, t) => new { t.BankProviderCode, t.Amount.Currency, r.RefundAmount })
            .ToListAsync(ct);
